...
%CONTAINER is the width of the bin that contain
% all the six bounding boxes of interest

%CHK will determine whether
% bounding boxes are y-dimension width's wise grouped or y-coordinate wise
% grouped.
%CHK=2 considers y-dimension width grouping and CHK=1 considers
% y-coordinate grouping.


takethisbox=[]; 
disp('outcome of function takeboxes');

for i=1:size(NR,1)
    if NR(i,(2*chk))>=container(1) && NR(i,(2*chk))<=container(2) 
        takethisbox=cat(1,takethisbox,NR(i,:));
        % concatenate along the row cause 1=row.
        disp(takethisbox);
    end
end
r=[];
